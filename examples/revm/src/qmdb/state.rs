...
impl QmdbState {
    ...
    pub(crate) fn database(&self) -> Result<QmdbRefDb, Error> {
        ...
        Ok(QmdbRefDb { inner: Arc::new(wrapped) })
    }
    ...
    pub(crate) async fn get_account(
        ...
        stores.accounts.get(&super::keys::account_key(address)).await.map_err(Error::from)
    }
    ...
    pub(crate) async fn get_code(&self, code_hash: B256) -> Result<Option<Vec<u8>>, Error> {
        ...
        let inner = self.inner.lock().await;
        let stores = inner.stores()?;
        stores.code.get(&super::keys::code_key(code_hash)).await.map_err(Error::from)
    }
    ...
    pub(crate) async fn get_storage(&self, address: Address, index: U256) -> Result<U256, Error> {
        ...
        let inner = self.inner.lock().await;
        let stores = inner.stores()?;
        let record = stores.accounts.get(&super::keys::account_key(address)).await?;
...
fn state_root_from_stores(stores: &Stores) -> StateRoot {
    state_root_from_roots(stores.accounts.root(), stores.storage.root(), stores.code.root())
}
