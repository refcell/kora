...
use super::{
    AccountStore, AccountStoreDirty, CodeStore, CodeStoreDirty, Error, QmdbChangeSet, StorageStore,
    StorageStoreDirty,
    keys::{AccountKey, CodeKey, StorageKey, account_key, code_key, storage_key},
...
impl StoresSlot {
    ...
    pub(super) fn get(&self) -> Result<&Stores, Error> {
        self.0.as_ref().ok_or(Error::StoreUnavailable("stores unavailable"))
    }

    /// Takes ownership of the stores while an update is in progress.
    pub(super) fn take(&mut self) -> Result<Stores, Error> {
        self.0.take().ok_or(Error::StoreUnavailable("stores unavailable"))
    }
...
impl QmdbInner {
    ...
    pub(crate) const fn new(stores: Stores) -> Self {
        Self { stores: StoresSlot::new(stores) }
    }
...
impl Stores {
    ...
    pub(crate) async fn apply_genesis(
        ...
        let Self { accounts, storage, code } = self;
...
impl DirtyStores {
    ...
    pub(super) async fn build_batches(
        ...
        for ((address, update), existing) in account_updates.into_iter().zip(existing_accounts) {
            ...
            let base_generation =
                existing.as_ref().map(|record| record.storage_generation).unwrap_or(0);
    ...
    pub(super) async fn apply_batches(&mut self, batches: StoreBatches) -> Result<(), Error> {
        self.accounts.write_batch(batches.accounts.into_iter()).await?;
        self.storage.write_batch(batches.storage.into_iter()).await?;
